...
- **限值边界唯一口径**: 超标判定集中在 `backend/app/domain/exceedance_rules.py` 的 `is_exceeded_ratio()` 一处, 一律按**未舍入的真实倍数**严格大于 1 判定(等价于监测值严格大于限值); **监测值恰好等于限值(倍数恰为 1)时结论为达标**, 不生成超标记录。`exceed_ratio` 仅在判定之后做三位舍入用于展示/导出, 任何统计、筛选、分级都不再拿舍入后的倍数反推是否超标, 因此明细、看板、查询统计与 CSV 导出对同一批数据给出同一个超标数。分级阈值(倍数 ≥ 1.5 中度、≥ 2 重度)仍取“不小于即升档”。
